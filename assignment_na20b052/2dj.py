...
while True:    
    ...
    if change < tolerance or iterations > 1700:
...
if rank == size-1:
    t_end = time.time()
    print("Time taken for convergence is", t_end - t_Start)
...
#     axs.plot(tx, [i for phi in phig for i in phi], label="Parallel", linewidth=3)
#     phix = np.loadtxt("phi_x.txt")
#     axs.plot(tx, phix, label="Serial", linestyle='--')
#     axs.set_xlabel('x')
...
# if rank == 2:
#     fig, axs = plt.subplots(figsize=(8, 12))
#     axs.plot(y, phi[0], label="Parallel", linewidth=3)
#     phiy = np.loadtxt("phi_y.txt")
#     axs.plot(y, phiy, label="Serial", linestyle='--') 
#     axs.set_xlabel('y')
